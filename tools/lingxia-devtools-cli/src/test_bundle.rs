...
fn bundle_test_entries(entries: &[PathBuf], identity: &Path) -> Result<TestBundle> {
    ...
    // Share one graph so dependencies execute once across all directory entries.
    for entry in entries {
...
mod tests {
    ...
    #[test]
    fn directory_bundle_never_touches_the_former_entry_path() {
        let dir = project();
        write(&dir, "one.test.ts", "globalThis.__one = true;\n");
        // A directory at the old filename makes even a transient write fail.
    ...
    #[test]
    fn directory_entries_share_dependencies_and_preserve_source_locations() {
        let dir = project();
        write(
            &dir,
            "helpers/shared.ts",
            "globalThis.__sharedOnce = true;\nexport const value = 7;\n",
        );
        write(
            &dir,
            "tests/a.test.ts",
            "import { value } from '../helpers/shared';\nglobalThis.__first = value;\n",
        );
        write(
            &dir,
        ...
        assert!(
            bundle.code.find("__sharedOnce = true").unwrap()
                < bundle.code.find("__first =").unwrap()
        );
        assert!(bundle.code.find("__first =").unwrap() < bundle.code.find("nested-").unwrap());
        let line = bundle
            .code
            .lines()
            .position(|line| line.contains("nested-"))
            .unwrap();
        let (_, frame) = bundle.remap_stack(&format!("at {}:{}:1", bundle.bundle_name, line + 1));
        let frame = frame.expect("original source position");
        assert_eq!(frame.source, "tests/nested/z.test.ts");
        assert_eq!(frame.line, 2);
    }

    #[test]
    fn concurrent_directory_bundles_are_identical() {
        let dir = project();
        write(&dir, "one.test.ts", "globalThis.__one = true;\n");

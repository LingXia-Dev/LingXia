...
/// Collect `*.test.ts` files under `root`, sorted by stable relative path.
pub fn collect_test_files(root: &Path) -> Result<Vec<PathBuf>> {
    ...
    collect_test_files_rec(&root, &mut files)?;
    files.sort_by(|left, right| {
        relative_display(left, &root)
            .replace('\\', "/")
            .cmp(&relative_display(right, &root).replace('\\', "/"))
    });
    if files.is_empty() {
        bail!("no *.test.ts files under {}", root.display());
    }
    Ok(files)
}

fn collect_test_files_rec(dir: &Path, out: &mut Vec<PathBuf>) -> Result<()> {
    ...
    for entry in entries {
        ...
        if path.is_dir() {
            ...
            collect_test_files_rec(&path, out)?;
            continue;
        }
...
/// Synthetic entry that imports every collected test file from `from_dir`.
pub fn synthetic_entry_source(files: &[PathBuf], from_dir: &Path) -> String {
    let from = normalize_path(from_dir)
        .map(|path| strip_verbatim(&path))
        .unwrap_or_else(|_| strip_verbatim(from_dir));
    let mut source = String::from("// Generated by lxdev test — one bundle, one run.\n");
    for file in files {
        ...
        let relative = file
            ...
            .replace('\\', "/");
        let specifier = if relative.starts_with("./") || relative.starts_with("../") {
            relative
        } else {
            format!("./{relative}")
        };
...
pub fn bundle_test_path(path: &Path) -> Result<TestBundle> {
    ...
    if path.is_dir() {
        ...
        let source = synthetic_entry_source(&files, &path);
        let entry = path.join(".lxdev-synthetic-entry.ts");
        fs::write(&entry, &source)
            .with_context(|| format!("failed to write {}", entry.display()))?;
        let result = bundle_test_entry(&entry);
...
globalThis.__RONG_TEST_HOST__ = {{\n\
  ...
  report: (event) => __lx_automation_host.emit(event),\n\
    ...
    code.push_str(
        "const __lx_test_framework = globalThis.__LINGXIA_TEST__ && typeof globalThis.__LINGXIA_TEST__.run === \"function\"\n\
  ? globalThis.__LINGXIA_TEST__\n\
  : globalThis.__RONG_TEST__;\n\
...
fn source_map_js_literal(map: &SourceMap<'_>) -> String {
    let raw = map.to_json_string();
...
pub(crate) fn find_project_root(entry: &Path) -> PathBuf {
    let fallback = entry
        .parent()
...
mod tests {
    ...
    #[test]
    fn directory_lists_test_files_in_stable_relative_order() {
        let dir = project();
        write(&dir, "z.test.ts", "export {};\n");
        write(&dir, "a.test.ts", "export {};\n");
        write(&dir, "nested/b.test.ts", "export {};\n");
        write(&dir, "skip.ts", "export {};\n");
        write(&dir, "notes.test.js", "export {};\n");

        let files = collect_test_files(dir.path()).expect("collect");
        let root = strip_verbatim(&normalize_path(dir.path()).expect("root"));
        let names = files
            .iter()
            .map(|path| relative_display(&strip_verbatim(path), &root).replace('\\', "/"))
            .collect::<Vec<_>>();
        assert_eq!(names, vec!["a.test.ts", "nested/b.test.ts", "z.test.ts"]);

        let source = synthetic_entry_source(&files, dir.path());
        let a = source.find("a.test.ts").expect("a");
        let nested = source.find("nested/b.test.ts").expect("nested");
        let z = source.find("z.test.ts").expect("z");
        assert!(a < nested && nested < z, "{source}");
        assert!(source.contains("import \"./a.test.ts\""));
    }

    #[test]
    fn prefers_lingxia_test_and_falls_back_to_rong() {
        let dir = project();
        let entry = write(&dir, "flow.ts", "export {};\n");
        let bundle = bundle_test_entry(&entry).expect("bundle");
        assert!(bundle.code.contains("__LINGXIA_TEST__"));
        assert!(bundle.code.contains("__RONG_TEST__"));
        assert!(bundle.code.contains("__LINGXIA_TEST_SOURCE_MAP__"));
        assert!(bundle.code.contains("__LINGXIA_CLI_VERSION__"));
        let lingxia = bundle.code.find("__LINGXIA_TEST__ &&").unwrap();
        let rong = bundle.code.find(": globalThis.__RONG_TEST__").unwrap();
        assert!(lingxia < rong);
    }

    #[test]
    fn directory_bundle_imports_every_test_file() {
        let dir = project();
    ...
    #[test]
    fn still_bundles_rong_fallback_registration() {
        let dir = project();
        write(
            &dir,
            "node_modules/@rongjs/test/package.json",
        ...
        let bundle = bundle_test_entry(&entry).expect("bundle rong");
        assert!(
            bundle.code.find("__RONG_TEST_HOST__").unwrap()
                < bundle.code.find("globalThis.__RONG_TEST__ =").unwrap()
        );

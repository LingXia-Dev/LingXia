...
//!
//! Credentials are resolved store-first, env-fallback so the same flow works
//! both for an interactively configured developer machine and a
//! non-interactive CI runner:
//!
//! - **Developer ID certificate** is read from the credential store
//!   (`~/.lingxia/apple/developer-id.json`, populated by
//!   `lingxia auth apple import-developer-id`) first, falling back to the
//!   `LINGXIA_APPLE_DEVELOPER_ID_P12` / `_P12_PASSWORD` / `_IDENTITY` env vars.
//! - **Notary (App Store Connect API) credentials** are read from the auth
//!   credential store (`~/.lingxia/apple/credentials.json`, populated by
//!   `lingxia auth apple login --mode key`) first, falling back to the
//!   `LINGXIA_APPLE_NOTARY_KEY` (`.p8` path) / `_KEY_ID` / `_ISSUER_ID` env vars.
//!
//! Signing + notarization runs only when BOTH halves resolve. When either is
//! unavailable this is a no-op and the app is left ad-hoc signed (the SwiftPM
//! default), which keeps local builds and tests green.
//!
//! Optional env vars:
...
pub fn maybe_sign_and_notarize(app_path: &Path) -> Result<()> {
    let developer_id = resolve_developer_id()?;
    let notary = resolve_notary()?;

    let (Some(developer_id), Some(notary)) = (developer_id, notary) else {
        println!(
            "{} macOS app left ad-hoc signed — provide a Developer ID cert \
             ('lingxia auth apple import-developer-id' or {}) AND notary creds \
             ('lingxia auth apple login --mode key' or {}/_KEY_ID/_ISSUER_ID) \
             to Developer-ID sign + notarize",
            "ℹ️".blue(),
            ENV_P12,
            ENV_NOTARY_KEY
        );
    ...
    let config = NotarizeConfig {
        developer_id,
        notary,
        entitlements: non_empty_env(ENV_ENTITLEMENTS),
    };

    sign_and_notarize(app_path, &config)
}
...
/// result.
fn notarize(app_path: &Path, config: &NotarizeConfig) -> Result<()> {
    if !Path::new(&config.notary.notary_key).exists() {
        return Err(anyhow!(
            "Notary key does not point to an existing file: {}",
            config.notary.notary_key
        ));
    }
    ...
    let result = (|| {
        ...
        let output = Command::new("xcrun")
            ...
            .arg("submit")
            .arg(&zip_path)
            .args(["--key", &config.notary.notary_key])
            .args(["--key-id", &config.notary.notary_key_id])
            .args(["--issuer", &config.notary.notary_issuer_id])
            .arg("--wait")
            .output()

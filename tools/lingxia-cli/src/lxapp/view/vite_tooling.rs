...
pub(super) fn ensure_project_tooling(
    ...
    if node_modules_dir.exists() {
        ensure_lingxia_workspace_built(&project.root)?;
        return Ok(None);
    }
...
/// Bootstrap the in-repo `@lingxia/*` workspace on a clean checkout: `npm
/// install` + `npm run build --workspaces` in `packages/` link and build every
/// member (npm discovers them — no per-package list), producing the `dist` the
/// view build needs. Kept under `packages/`, not the repo root, so `examples/`
/// apps still install standalone like a `lingxia new` project. Runs once,
/// gated on `bridge/dist`.
fn ensure_lingxia_workspace_built(project_root: &Path) -> Result<()> {
    let Some(repo_root) = find_lingxia_workspace_root(project_root) else {
        return Ok(());
    };
    let packages_dir = repo_root.join("packages");
    let bridge_dist = packages_dir.join("lingxia-bridge").join("dist");
    let built = fs::read_dir(&bridge_dist)
    ...
    println!("  ▸ installing LingXia workspace packages");
    run_workspace_npm(
        &packages_dir,
        &["install", "--no-audit", "--no-fund"],
        "install LingXia workspace packages",
    )?;
    println!("  ▸ building LingXia workspace packages");
    run_workspace_npm(
        &packages_dir,
        &["run", "build", "--workspaces", "--if-present"],
        "build LingXia workspace packages",
    )
}

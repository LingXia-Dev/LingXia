...
pub fn execute(options: BuildExecuteOptions) -> Result<()> {
    ...
    // A CLI / package version skew fails here, not after the build.
    crate::compat::ensure_project(&current_dir, &[])?;
    ...
    if available_platforms.is_empty() {
        return Err(anyhow!(
            "No platform configured in lingxia.config.json.\n\
             Set app.platforms to include at least one of: android, ios, macos, harmony, windows"
        ));
    }

...
fn generate_rust(
    ...
    let mut err_code_entries: Vec<(u32, String)> = Vec::new();
    for key in all_keys.keys() {
        if let Some(code_str) = key.strip_prefix("err_code_")
            && code_str.chars().all(|c| c.is_ascii_digit())
            && let Ok(code) = code_str.parse::<u32>()
        {
            err_code_entries.push((code, key.to_pascal_case()));
        }
    }
    err_code_entries.sort_by_key(|(code, _)| *code);
...
",
    ...
    content.push_str("        }\n");
    content.push_str("    }\n\n");
    content.push_str("    pub fn from_err_code(code: u32) -> Option<Self> {\n");
    if err_code_entries.is_empty() {
        content.push_str("        let _ = code;\n");
        content.push_str("        None\n");
    } else {
        content.push_str("        match code {\n");
        for (code, variant_name) in err_code_entries {
            content.push_str(&format!(
                "            {} => Some(I18nKey::{}),\n",
                code, variant_name
            ));

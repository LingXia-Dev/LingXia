...
struct RunnerBuildTool {
    ...
    private static func run() throws {
        ...
        let staticLibPath = try buildRustLibrary(
        ...
        // SwiftPM does not track the external Rust `liblingxia.a` as a link input,
        // so it would not relink when only the Rust code changes. Emitting a
        // generated Swift source whose content is the staticlib's hash makes
        // SwiftPM recompile and relink whenever the library actually changes.
        try writeStaticLibFingerprint(libraryPath: staticLibPath, outputDir: options.outputDir)
    ...
    /// Write a generated Swift file whose content is the staticlib's content hash.
    /// Declared as a build-tool plugin output, SwiftPM compiles it into the target,
    /// so a changed `.a` (changed hash) forces a recompile and relink. Rewritten
    /// only when the hash changes, so unrelated builds do not trigger relinks.
    private static func writeStaticLibFingerprint(libraryPath: String, outputDir: String) throws {
        let data = try Data(contentsOf: URL(fileURLWithPath: libraryPath))
        let hash = SHA256.hash(data: data).map { String(format: "%02x", $0) }.joined()
        let path = (outputDir as NSString).appendingPathComponent("RustLibFingerprint.swift")
        let contents = """
        // Generated by RunnerBuildTool — do not edit.
        // Forces SwiftPM to relink LingXiaRunner when liblingxia.a changes.
        let _rustLibFingerprint = "\(hash)"

        """
        if let existing = try? String(contentsOfFile: path, encoding: .utf8), existing == contents {
            return
        }

...
impl lingxia::HostAddon for RunnerDevtoolAddon {
    // Cloud provider. Registered here — the logic context is built before
    // `start_services`. Injected via `--with-provider cloud`. Mirrors the macOS
    // runner (tools/lingxia-runner/macos/native/src/lib.rs).
    #[cfg(feature = "cloud")]

...
spec("apply navigationBar title, colors, home button, and reset", {
  ...
  await t.step('drive the ui page presets', async () => {
    ...
    await raw.page.waitFor({ page: 'ui', css: '[data-testid="navbar-preset-blue"]', state: 'visible' });
  ...
  await t.step('survive reLaunch', async () => {
    ...
    await app.nav.relaunch({ page: 'ui', query: { type: 'navbar' } });
    await waitForCurrentPage(raw, 'ui', 30_000);
    await raw.page.waitFor({ page: 'ui', css: '[data-testid="navbar-preset-blue"]', state: 'visible' });
...
spec("round-trip appearance preference through the ui controls", {
  ...
  defer(async () => {
    await raw.eval({ script: `await lx.host.control.appearance.setPreference('auto');` })
      .catch(() => undefined);
  });

  await app.nav.relaunch({ page: 'ui', query: { type: 'appearance' } });
  await raw.page.waitFor({ page: 'ui', css: '[data-testid="ui-appearance-light"]', state: 'visible' });
  ...
  await t.step('reject an invalid preference and keep the previous state', async () => {
    ...
    const rejected = await evalCaught(raw, `await lx.host.control.appearance.setPreference('sepia');`);
    expect(rejected.ok).toBeFalsy();
    expect(rejected.code).toBe('E_INVALID_ARG');
  ...
  await t.step('persist across relaunch', async () => {
    ...
    await relaunchFromLogic(raw, 'ui', { type: 'appearance' });
    await waitForCurrentPage(raw, 'ui', 30_000);
    await raw.page.waitFor({ page: 'ui', css: '[data-testid="ui-appearance-dark"]', state: 'visible' });
...
spec("show, hide, confirm, and cancel in-app feedback overlays", {
  ...
  await t.step('show and hide a toast', async () => {
    ...
    await raw.page.waitFor({ page: 'ui', css: '[data-testid="toast-show"]', state: 'visible' });
    await raw.eval({
      script: `await lx.showToast({ title: 'Coverage toast', icon: 'none', durationMs: 8000 });`,
    });
    if (overlays === 'dom') {
      await raw.page.waitFor({ page: 'ui', css: '.lx-toast-title', state: 'visible' });
      const shown = await raw.page.eval({
        page: 'ui',
      ...
      const leaked = await raw.page.eval({
        page: 'ui',
        script: `document.querySelector('.lx-toast-title') ? 'yes' : 'no'`,
      });
      expect(leaked).toBe('no');
    }
    await raw.eval({ script: `await lx.hideToast();` });
    if (overlays === 'dom') {
      await eventually(
        () => raw.page.eval({
          page: 'ui',
  ...
  await t.step('use acknowledgement and boolean dialogs', async () => {
    const alert = raw.eval({ script: `await lx.alert({ title: 'Notice' }); return 'acknowledged';` });
    await raw.page.waitFor({ page: 'ui', css: '.lx-modal-btn-confirm', state: 'visible' });
    await app.view.css('.lx-modal-btn-confirm', { page: 'ui' }).click();
    expect(await alert).toBe('acknowledged');
    const confirmed = raw.eval({ script: `return await lx.confirm({ title: 'Continue?' });` });
    await raw.page.waitFor({ page: 'ui', css: '.lx-modal-btn-confirm', state: 'visible' });
    await app.view.css('.lx-modal-btn-confirm', { page: 'ui' }).click();
    expect(await confirmed).toBe(true);
    const canceled = raw.eval({ script: `return await lx.confirm({ title: 'Continue?' });` });
    await raw.page.waitFor({ page: 'ui', css: '.lx-modal-btn-cancel', state: 'visible' });
    await app.view.css('.lx-modal-btn-cancel', { page: 'ui' }).click();
  ...
  await t.step('pick and dismiss an action sheet', async () => {
    ...
    await raw.page.waitFor({ page: 'ui', css: '.lx-as-item', state: 'visible' });
    ...
    const dismissed = raw.eval({
      script: `return await lx.showActionSheet({ items: ['One', 'Two'].map((label) => ({ id: label, label })) });`,
    }) as Promise<{ status: 'ok' | 'canceled' }>;
    await raw.page.waitFor({ page: 'ui', css: '.lx-as-cancel-btn', state: 'visible' });
  ...
  await t.step('reject an empty action sheet', async () => {
    const rejected = await evalCaught(raw, `await lx.showActionSheet({ items: [].map((label) => ({ id: label, label })) });`);
    expect(rejected.ok).toBeFalsy();
    expect(rejected.code).toBe('E_INVALID_ARG');
  });
});
...
spec("assert tabBar failure codes, resets, and button-driven patches", {
  ...
  await t.step('reject invalid updates with E_INVALID_ARG', async () => {
    ...
    for (const body of [
      ...
      const rejected = await evalCaught(raw, body);
      expect(rejected.ok).toBeFalsy();
      expect(rejected.code).toBe('E_INVALID_ARG');
      expect(await tabBar(app)).toEqual(before);
    }
  ...
  await t.step('switchTab agrees with selected_index after a badge change', async () => {
    await raw.eval({ script: `void lx.switchTab({ page: 'todo' }); return 'scheduled';` });
    await waitForCurrentPage(raw, 'todo');

    ...
    <div class="px-4 py-4 space-y-3 pb-8">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          <div class="w-8 h-8 bg-linear-to-br from-orange-500 to-rose-500 rounded-lg flex items-center justify-center">
        ...
        <div class="bg-surface-900 text-emerald-400 font-mono text-xs px-3 py-1.5 rounded-lg w-[180px] truncate">
          {{ eventLog }}
        </div>
      </div>
      ...
      <!-- Source -->
      <div class="bg-surface rounded-xl shadow-sm border border-line-200 p-4 space-y-3">
        <div class="text-xs text-gray-400 uppercase tracking-wider font-semibold">Add Media</div>
        <div class="grid grid-cols-2 gap-2">
          <button
            @click="pickImages?.()"
            :disabled="busy"
            class="px-3 py-2.5 rounded-lg bg-linear-to-r from-blue-500 to-cyan-500 text-white text-sm font-medium active:scale-95 disabled:opacity-50 transition-transform"
          >
            Add Image
          </button>
          <button
            @click="pickVideos?.()"
            :disabled="busy"
            class="px-3 py-2.5 rounded-lg bg-linear-to-r from-purple-500 to-pink-500 text-white text-sm font-medium active:scale-95 disabled:opacity-50 transition-transform"
          >
            Add Video
          </button>
        </div>
      ...
      <div class="bg-surface rounded-xl shadow-sm border border-line-200 p-4 space-y-3">
        ...
        <div class="grid grid-cols-4 gap-2">
          <button
            ...
            class="px-3 py-2 rounded-lg bg-surface-100 text-gray-700 text-sm font-medium active:scale-95 disabled:opacity-40"
          >
            Prev
          </button>
          <button
            @click="callNext"
            :disabled="items.length < 2"
            class="px-3 py-2 rounded-lg bg-surface-900 text-white text-sm font-medium active:scale-95 disabled:opacity-40"
          >
            Next
          </button>
          <button
            ...
            class="px-3 py-2 rounded-lg bg-amber-50 text-amber-700 dark:text-amber-400 text-sm font-medium active:scale-95 disabled:opacity-40"
          >
            Remove
          </button>
          <button
            @click="clearAll?.()"
            :disabled="items.length === 0"
            class="px-3 py-2 rounded-lg bg-rose-50 text-rose-700 dark:text-rose-400 text-sm font-medium active:scale-95 disabled:opacity-40"
          >
            Clear
          </button>
        </div>
      ...
      <div class="bg-surface rounded-xl shadow-sm border border-line-200 p-4 space-y-3">
        ...
        <div class="flex items-center justify-between">
          ...
          <button
            ...
            :class="[
              ...
              autoplay ? 'bg-emerald-500' : 'bg-surface-300',
            ]"
          >
            <span
              :class="[
        ...
        <div class="flex items-center justify-between">
          ...
          <button
            ...
            :class="[
              'w-11 h-6 rounded-full relative transition-colors',
              loop ? 'bg-emerald-500' : 'bg-surface-300',
            ]"
          >
            <span
              :class="[
        ...
        <div class="flex items-center justify-between">
          ...
          <button
            ...
            :class="[
              'w-11 h-6 rounded-full relative transition-colors',
              dots ? 'bg-emerald-500' : 'bg-surface-300',
            ]"
          >
            <span
              :class="[
        ...
        <div class="space-y-2">
          ...
          <div class="grid grid-cols-3 gap-1 bg-surface-100 rounded-lg p-1">
            <button
              v-for="fit in fits"
              :key="fit"
              ...
              :class="[
                ...
                objectFit === fit ? 'bg-surface text-gray-900 shadow-sm' : 'text-gray-500',
              ]"
            >
              {{ fit }}
            </button>
        ...
        <div class="space-y-2">
          ...
          <div class="grid grid-cols-2 gap-1 bg-surface-100 rounded-lg p-1">
            <button
              ...
              :class="[
                'py-1.5 px-2 rounded-md text-xs font-medium transition-colors',
                animation === a ? 'bg-surface text-gray-900 shadow-sm' : 'text-gray-500',
              ]"
            >
              {{ a }}
            </button>
        ...
        <div class="space-y-2">
          ...
          <div class="grid grid-cols-2 gap-1 bg-surface-100 rounded-lg p-1">
            <button
              ...
              :class="[
                'py-1.5 px-2 rounded-md text-xs font-medium transition-colors',
                direction === d ? 'bg-surface text-gray-900 shadow-sm' : 'text-gray-500',
              ]"
            >
              {{ d }}
            </button>
        ...
        <div class="space-y-2">
          ...
          <div class="grid grid-cols-4 gap-1 bg-surface-100 rounded-lg p-1">
            <button
              ...
              :class="[
                'py-1.5 px-2 rounded-md text-xs font-medium transition-colors',
                peek === value ? 'bg-surface text-gray-900 shadow-sm' : 'text-gray-500',
              ]"
            >
      ...
      <!-- Status -->
      <div class="bg-surface rounded-xl shadow-sm border border-line-200 px-4 py-3 flex items-center justify-between text-xs">

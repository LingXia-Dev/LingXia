...
if [ "$SKIP_RUST" = false ]; then
  echo "[1/3] Building Rust (HarmonyOS target) ..."
  ...
  (cd "$LINGXIA_ROOT" && cargo build --release --target=aarch64-unknown-linux-ohos -p lingxia-lib)
else
  echo "[1/3] Using existing liblingxia.so"
fi
SO_SRC="$LINGXIA_ROOT/target/aarch64-unknown-linux-ohos/release/liblingxia.so"

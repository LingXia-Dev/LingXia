...
impl DownloadsStore {
    fn open(db_path: &Path) -> Result<Self, LxAppError> {
    ...
    fn snapshot(&self, app_data_dir: &Path) -> Result<DownloadsSnapshot, LxAppError> {
    ...
    fn get_record_reconciled(&self, task_id: &str) -> Result<Option<DownloadRecord>, LxAppError> {
    ...
    fn remove_record(&self, task_id: &str) -> Result<(), LxAppError> {
        let txn = self
            .db
            .begin_write()
            .map_err(|e| db_error("begin write transaction", e))?;
    ...
    fn clear_completed(&self) -> Result<u64, LxAppError> {
    ...
    fn persist_record(&self, record: &DownloadRecord) -> Result<(), LxAppError> {
    ...
    fn reconciled_records(&self) -> Result<Vec<DownloadRecord>, LxAppError> {
        self.records
            .iter()
            .map(|entry| entry.value().clone())
            .collect::<Vec<_>>()
...
pub(crate) fn record_bridge_event(
    app_data_dir: &Path,
    event_name: &str,
    payload: &Value,
) -> Result<(), LxAppError> {
    ...
    match event_name {
        BRIDGE_EVENT_STARTED => {
            ...
            let owner = browser_download_owner(&started.tab_id);
            let now = unix_ms_now();
            let resume = store
        ...
        BRIDGE_EVENT_COMPLETED => {
            ...
            let mut record =
                store
                    ...
                    .unwrap_or_else(|| DownloadRecord {
                        ...
                        owner: browser_download_owner(&completed.tab_id),
            ...
            let now = unix_ms_now();
            record.owner = browser_download_owner(&completed.tab_id);
        ...
        BRIDGE_EVENT_FAILED => {
            ...
            if let Some(mut record) = store.get_record(&failed.task_id) {
                record.owner = browser_download_owner(&failed.tab_id);
...
pub(crate) fn snapshot(app_data_dir: &Path) -> Result<DownloadsSnapshot, LxAppError> {
    let store = downloads_store(app_data_dir)?;
...
pub(crate) fn get_record(
    app_data_dir: &Path,
    task_id: &str,
) -> Result<Option<DownloadRecord>, LxAppError> {
    let store = downloads_store(app_data_dir)?;
    store.get_record_reconciled(task_id)
}

pub(crate) fn get_request_context(
    app_data_dir: &Path,
    task_id: &str,
) -> Result<Option<DownloadRequestContext>, LxAppError> {
    let store = downloads_store(app_data_dir)?;
...
pub(crate) fn load_resume_metadata(
    app_data_dir: &Path,
    task_id: &str,
) -> Result<Option<ResumeMetadata>, LxAppError> {
    let store = downloads_store(app_data_dir)?;
    Ok(store
...
pub(crate) fn clear_resume_metadata(app_data_dir: &Path, task_id: &str) -> Result<(), LxAppError> {
    let store = downloads_store(app_data_dir)?;
...
pub(crate) fn clear_completed(app_data_dir: &Path) -> Result<u64, LxAppError> {
    let store = downloads_store(app_data_dir)?;
...
fn downloads_store(app_data_dir: &Path) -> Result<Arc<DownloadsStore>, LxAppError> {
...
pub(crate) fn register_active_download(task_id: &str) -> watch::Receiver<bool> {
    let (tx, rx) = watch::channel(false);
    active_download_cancels().insert(task_id.to_string(), tx);
    rx
}

pub(crate) fn unregister_active_download(task_id: &str) {
    active_download_cancels().remove(task_id);
}

pub(crate) fn cancel_active_download(task_id: &str) -> bool {
...
pub(crate) fn has_active_download(task_id: &str) -> bool {
    is_active_download(task_id)
}

fn downloads_db_path(app_data_dir: &Path) -> PathBuf {
    crate::app::state::file(app_data_dir, "downloads.redb")
}
...
fn db_error(action: &str, err: impl std::fmt::Display) -> LxAppError {
    LxAppError::Runtime(format!("downloads store {action} failed: {err}"))
}

fn browser_download_owner(tab_id: &str) -> DownloadOwner {
    DownloadOwner {
        kind: DownloadOwnerKind::Browser,
        appid: crate::browser::BUILTIN_BROWSER_APPID.to_string(),
        page_path: Some(crate::browser::browser_tab_path_for_id(tab_id)),

...
export class PageProcessor {
  ...
  constructor(projectPath: string, outputDir: string, staticDirs: string[] = DEFAULT_STATIC_DIRS) {
    this.projectPath = projectPath;
    this.outputDir = outputDir;
    this.fileUtils = new FileUtils();
    this.templateManager = new TemplateManager();
    this.framework = readProjectFramework(projectPath);
    const lingxiaConfig = loadLingxiaConfig(projectPath);
    const viewOverrides = extractViewOverrides(lingxiaConfig, this.framework);
    this.pluginSpecs = extractPluginSpecs(lingxiaConfig);
    this.viewConfigManager = new ViewConfigManager(projectPath, viewOverrides);
  ...
  private createProjectRequire(projectPath: string): NodeJS.Require {
    const candidateFiles = ['package.json', 'lingxia.config.ts', 'lingxia.config.js'];

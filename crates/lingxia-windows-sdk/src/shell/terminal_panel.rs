...
fn create_panel_session(panel_id: &str) -> u64 {
    ...
    let (cols, rows) = super::terminal_grid::desired_grid_size(panel_id).unwrap_or((100, 24));
...
fn run_terminal_panel_poll_loop(panel_key: &str, stop: &AtomicBool) {
    ...
    loop {
        ...
        #[cfg(feature = "shell-runtime")]
        {
            ...
            let desired = super::terminal_grid::desired_grid_size(panel_key)
        ...
        if switched
            || last_generations.get(&session_id) != Some(&generations)
            || refresh_tick % 25 == 0
        {
            publish_windows_terminal_snapshot(panel_key, snapshot);

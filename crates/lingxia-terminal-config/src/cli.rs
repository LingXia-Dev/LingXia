...
/// How the platform lists installed fonts. Registered by the host before it
/// dispatches the CLI, because enumerating families is platform work.
pub type FontLister = fn() -> Vec<crate::InstalledFont>;

static FONT_LISTER: std::sync::OnceLock<FontLister> = std::sync::OnceLock::new();

/// Let the CLI list fonts. Called once by the host at startup.
pub fn register_font_lister(lister: FontLister) {
    let _ = FONT_LISTER.set(lister);
}

fn installed_fonts() -> Vec<crate::InstalledFont> {
    FONT_LISTER.get().map(|lister| lister()).unwrap_or_default()
}

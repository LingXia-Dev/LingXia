...
use corner_caps::{
    destroy_screen_corner_caps, raise_screen_corner_caps, update_screen_corner_caps,
};
...
/// move/size loop. It mirrors the webview host's live-layout cadence but is
...
pub(super) fn set_webview_device_frame(
    ...
    let host_window = find_webview_host_window(webtag).map_err(|err| err.to_string())?;
...
fn apply_screen_corners(content: HWND, spec: &WindowsDeviceFrame) {
    ...
    destroy_screen_corner_caps(content);
    if radius > 0 {
        unsafe {
...
fn sync_device_frame_for_content(content: HWND) {
    ...
    raise_screen_corner_caps(content);
}
...
unsafe extern "system" fn device_frame_host_proc(
    ...
    if msg == WindowsAndMessaging::WM_WINDOWPOSCHANGED {
        ...
        destroy_screen_corner_caps(hwnd);
...
fn request_host_window_layout(hwnd: HWND) {
    let _ = request_webview_host_window_layout(WindowsWebViewHostWindow {

...
///
/// Native and browser main surfaces keep the home lxapp as their control app,
/// so its lifecycle runs even when no lxapp WebView is mounted.
...
pub fn init(platform: Platform) -> crate::Result<crate::RuntimeInfo> {
    ...
    lingxia_platform::set_toast_activate_handler(|token| {
        let _ = crate::navigation::activate_notification(token);
    });

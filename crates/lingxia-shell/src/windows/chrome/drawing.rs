...
pub(in crate::windows) fn create_chrome_text_font(hdc: HDC) -> HFONT {
    unsafe {
        CreateFontW(
...
pub(in crate::windows) fn draw_text(
    hdc: HDC,
    text: &str,
    rect: RECT,
    rgb: u32,
...
pub(in crate::windows) fn logical_font_height(hdc: HDC, point_size: i32) -> i32 {
    let dpi_y = unsafe { GetDeviceCaps(Some(hdc), LOGPIXELSY) };
    let dpi_y = if dpi_y > 0 { dpi_y } else { 96 };
    (point_size * dpi_y + 36) / 72
}

pub(in crate::windows) fn draw_badge(hdc: HDC, item_rect: RECT, badge: &str) {
    let badge_rect = RECT {
...
pub(in crate::windows) fn draw_red_dot(hdc: HDC, item_rect: RECT) {
    let dot_rect = RECT {
...
pub(in crate::windows) fn draw_top_border(hdc: HDC, rect: RECT, rgb: u32) {
    fill_rect(
        hdc,
        RECT {
            left: rect.left,
...
pub(in crate::windows) fn draw_bottom_border(hdc: HDC, rect: RECT, rgb: u32) {
    fill_rect(
        hdc,
        RECT {
            left: rect.left,
...
pub(in crate::windows) fn draw_left_border(hdc: HDC, rect: RECT, rgb: u32) {
    fill_rect(
        hdc,
        RECT {
            left: rect.left,
...
pub(in crate::windows) fn draw_right_border(hdc: HDC, rect: RECT, rgb: u32) {
    fill_rect(
        hdc,
        RECT {
            left: rect.right - 1,
...
pub(in crate::windows) fn fill_rect(hdc: HDC, rect: RECT, rgb: u32) {
...
pub(in crate::windows) fn fill_round_rect_aa(hdc: HDC, rect: RECT, radius: i32, rgb: u32) {
...
pub(in crate::windows) fn centered_icon_rect(rect: RECT, size: i32) -> RECT {
...
pub(in crate::windows) fn draw_icon_from_path(hdc: HDC, path: &str, rect: RECT, size: u32) -> bool {
...
pub(in crate::windows) fn draw_icon_from_png_bytes(
    hdc: HDC,
    cache_key: &str,
    png: &[u8],
    rect: RECT,
...
pub(in crate::windows) fn rgb_to_colorref(rgb: u32) -> COLORREF {
    let r = (rgb >> 16) & 0xff;
    let g = (rgb >> 8) & 0xff;
    let b = rgb & 0xff;
    COLORREF(r | (g << 8) | (b << 16))

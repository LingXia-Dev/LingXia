//! Process-local URL callback channels for WebView navigation.
//!
//! A caller that needs to observe a specific navigation — and receive its URL
//! without loading it — opens a channel for that URL, then awaits it:
//!
//! ```no_run
//! # async fn wait_for() -> Result<(), lingxia_webview::url_callback::InvalidCallbackUrl> {
...
//! of loading, and the full URL is delivered to the channel. The callback URL
//! is a plain navigation-target string: no AppLink, OS custom-scheme
//! registration, or scheme handler is involved.
//!
//! Interception is process-wide — any web content able to navigate can hit the
//! callback URL — so the caller should tie the delivered URL back to the flow
//! that opened the channel (e.g. via a query parameter it planted) before
//! trusting it.

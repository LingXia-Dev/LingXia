//! Windows API callbacks and UI-thread dispatch.

use super::*;

pub(crate) type CloseHandler = Arc<dyn Fn() + Send + Sync>;

pub(crate) type ChromeEventHandler = Arc<dyn Fn(WindowsChromeCommand) + Send + Sync>;
...
pub(crate) static WINDOW_HOST_PANEL_INPUT_HANDLERS: OnceLock<
    Mutex<HashMap<String, WindowsHostPanelInputHandler>>,
> = OnceLock::new();

pub(crate) static WEBVIEW_USER_DATA_DIR: OnceLock<Mutex<Option<PathBuf>>> = OnceLock::new();

pub(crate) static HOST_WINDOW_CREATED_HANDLERS: OnceLock<Mutex<Vec<HostWindowCreatedHandler>>> =
    OnceLock::new();
...
/// Runs `callback` on the UI thread that owns `window` (a host-window
/// handle previously returned by this crate).
///
/// Generic thread-marshalling mechanics for host integrations that must
/// touch Win32 UI owned by a webview UI thread from a background thread.
/// Returns `false` when the window is gone or the post failed; the callback
...
pub fn set_host_panel_input_handler(panel_id: &str, handler: WindowsHostPanelInputHandler) {
    let handlers = WINDOW_HOST_PANEL_INPUT_HANDLERS.get_or_init(|| Mutex::new(HashMap::new()));
...
pub fn clear_host_panel_input_handler(panel_id: &str) {
    if let Some(handlers) = WINDOW_HOST_PANEL_INPUT_HANDLERS.get()
...
pub(crate) fn invoke_host_window_created_handler(hwnd: HWND) {
...
pub(crate) fn invoke_chrome_event_handler(webtag_key: &str, command: WindowsChromeCommand) -> bool {

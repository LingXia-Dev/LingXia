...
public class LingXiaWebView extends WebView implements LingXiaWebViewHost {
    ...
    public static void requestServoWebView(final String appId, final String path, final long sessionId, final String optionsToken) {
        ensureMainThreadStatic(new Runnable() {
            @Override
            public void run() {
                try {
                    ...
                    LingXiaServoView servoView = new LingXiaServoView(sApplicationContext);
                    servoView.initialize(
                            appId,
                            path,
                            sessionId,
                            "strict_default".equals(options.profile));
                    notifyWebViewReady(appId, path, sessionId, servoView);
                } catch (Throwable e) {

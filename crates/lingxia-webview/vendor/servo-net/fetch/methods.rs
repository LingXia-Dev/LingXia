...
use crate::devtools::{
    notify_failure, send_early_httprequest_to_devtools, send_response_to_devtools,
    send_security_info_to_devtools,
};
...
pub async fn main_fetch(
    ...
    send_response_to_devtools(request, context, &response, None, false);
    ...
    send_response_to_devtools(request, context, &response, None, true);
...
async fn wait_for_response(
    ...
    if let Some(ref mut ch) = *done_chan {
        let mut devtools_body =
            (context.devtools_chan.is_some() || crate::has_network_observer()).then(Vec::new);
        loop {
            match ch.1.recv().await {
                ...
                Some(Data::Done) => {
                    send_response_to_devtools(request, context, response, devtools_body, true);
                    break;
                }
                Some(Data::Cancelled) => {
                    response.aborted.store(true, Ordering::Release);
                    notify_failure(request, &NetworkError::LoadCancelled);
                    break;
                }
                _ => {
        ...
        match *response.actual_response().body.lock() {
            ResponseBody::Done(ref vec) if !vec.is_empty() => {
                ...
                if context.devtools_chan.is_some() || crate::has_network_observer() {
                    ...
                    // notify diagnostics observers with the full Response.
                    send_response_to_devtools(request, context, response, Some(vec.clone()), true);
                }
            }

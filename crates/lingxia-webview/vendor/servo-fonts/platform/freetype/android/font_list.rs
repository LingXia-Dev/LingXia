...
struct Font {
    ...
    weight: Option<i32>,
    style: Option<String>,
    /// Face index within a `.ttc` collection (LingXia patch).
    index: Option<u16>,
}
...
impl FontList {
    ...
    fn parse_family(familyset: &[Node], attrs: &[Attribute], out: &mut Vec<FontFamily>) {
        ...
        // Parse family name. LingXia patch: modern fonts.xml files declare
        // fallback families (CJK among them) only by `lang`; register those
        // as `lang:<tag>` so `fallback_font_families` can reach them.
        let names = if let Some(name) = Self::find_attrib("name", attrs) {
            vec![name]
        } else if let Some(langs) = Self::find_attrib("lang", attrs) {
            langs
                .split(',')
                .map(|lang| format!("lang:{}", lang.trim()))
                .collect()
        } else {
            return;
        };
    ...
    fn parse_font(nodes: &[Node], attrs: &[Attribute], out: &mut Vec<Font>) {
        ...
        if let Some(filename) = Self::text_content(nodes) {
            ...
            let index = Self::find_attrib("index", attrs).and_then(|i| i.parse().ok());
        ...
        match block {
            ...
            _ => {
                if is_cjk(options.character) {
                    ...
                    // LingXia patch: the `lang`-only CJK families of modern
                    // fonts.xml, preferring the content language's glyphs.
                    let preferred = match options.language.as_str() {
                        "ja" => "lang:ja",
                        "ko" => "lang:ko",
                        _ => "lang:zh-Hans",
                    };
                    ...
                    families.extend(
                        ["lang:zh-Hans", "lang:zh-Hant", "lang:ja", "lang:ko"]
                            .into_iter()
                            .filter(|family| *family != preferred),
                    );
                }
            },
        }

...
- **`lx.app.checkUpdate()` and `screenshot()` are home-lxapp only** (others get a permission error). `checkUpdate()` also **opts the whole app into custom update handling** — the built-in update UI is suppressed afterward. `apply()` (direct package handoff) works on Android/macOS; elsewhere point users at the store.

...
- **Appearance is host-owned.** Every lxapp may read `lx.getAppearance()` and subscribe with `lx.onAppearanceChange()`. Only the home lxapp may persist `lx.app.setAppearance({ preference: 'system' | 'light' | 'dark' })`; pages respond through CSS `prefers-color-scheme`, while native tab/navigation bars update automatically.

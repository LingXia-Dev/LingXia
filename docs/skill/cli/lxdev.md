...
- `reload` — rebuild the selected lxapp front-end bundle through the running session, then reload it so the new bundle is live (covers Web, Logic, and `lxapp.json` changes); use `--app <appid>` for a non-home resource and `--build-only` to skip the runtime reload

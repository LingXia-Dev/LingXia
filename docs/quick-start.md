...
The skill ships inside the CLI, so an installed copy always matches the `lingxia` that wrote it and cannot describe commands that version does not have. `lingxia new` prints a hint at the end of scaffolding but does not install the skill itself — run the command yourself when you want it:

```bash
lingxia skill install              # writes to ./.claude/skills/lingxia/
lingxia skill install --user       # writes to ~/.claude/skills/lingxia/
lingxia skill install --agents-md  # also writes AGENTS.md for Codex-style tools
lingxia skill install --from <dir> # install an edited skill without rebuilding
lingxia skill status               # where it is, and which CLI wrote it
```

A copy in `~/.claude/skills/` is rewritten when a newer `lingxia` runs, so upgrading the CLI upgrades the skill with it.
...
- **Claude Code, Claude Agent SDK** — use `lingxia skill install`; Claude discovers `.claude/skills/lingxia/SKILL.md`.
- **OpenAI Codex CLI** — use `lingxia skill install --agents-md`; Codex reads `<project>/AGENTS.md`, and the installer writes a pointer from that file to `.claude/skills/lingxia/SKILL.md`.

...
3.  **Logic Bridge Responds `reply` (Success - Sent Immediately):**
    ...
    ```
    *View receives reply, clears timer, handles success acknowledgment (e.g., disables button, shows 'Processing...'). View now waits for potential subsequent `setData` for final result.*
...
4.  **Logic Bridge Responds `reply` (Failure - Sent Immediately):**
    *(Alternative to step 3 if handler invocation failed immediately)*
    ...
    ```
    *View receives, clears timer, handles failure (e.g., re-enables button, displays `error.message`).*

5.  **Timeout:** If the timer expires before *any* `reply` arrives, the View handles the timeout error (e.g., "Request timed out - Logic layer unresponsive").
...
2.  **View Bridge Processes:** Receives the call, finds the `setData` handler. Attempts to invoke the handler with the payload. Assume the handler successfully starts applying the patch synchronously without immediate format errors.

3.  **View Bridge Responds `reply` (Success - Sent Immediately):**
    ```json
    {
      "msgId": "logic-1678886400567-10",
      "type": "reply",
    ...
    ```
    *Logic receives reply, clears timer, knows the View has accepted the `setData` request for processing.*
    *(Meanwhile, the `setData` handler in the View Layer continues applying the patch and potentially re-rendering asynchronously).*

4.  **View Bridge Responds `reply` (Failure - Sent Immediately):**
    *(Alternative to step 3 if handler invocation failed immediately)*
    ```json
    {
      "msgId": "logic-1678886400567-10",
      ...
      "payload": {
        "success": false,
        "error": { "message": "Failed to apply data update: Invalid patch format." }
      }
    }
    ```
    *Logic receives, clears timer, logs the error, potentially flags view desynchronization.*

5.  **Timeout:** If the timer expires before *any* `reply` arrives, Logic assumes the View failed to process the update request, logs an error, and handles potential desynchronization.
